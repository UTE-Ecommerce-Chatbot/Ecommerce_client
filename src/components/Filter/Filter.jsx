        ...
        <>
            <div className="col l-2-4 m-0 c-0" style={{paddingBottom: "50px"}}>
                            ...
                            return (
                                <li className="category-item category-item__filter" key={index}>
                                    <div className="item" onClick={() => {
                                        ...
                                        <span
                                            className={sortBy === item.sortBy && sortValue === item.sortValue ? 'selected' : ''}>{item.label}</span>
                                    </div>
                                </li>
                            );
                        })}
                ...
                <div className="filter__bar">
                    ...
                    <ul className="category-list">
                        {list_price
                            .filter((item) => item.category === category ? category : "dien-thoai")[0]
                            .prices.map((price, index) => {
                                return (
                                    <li className="category-item category-item__filter" key={index}>
                                        <div className="item" onClick={() => addQuery("price", price.value)}>
                                            <span
                                                className={price_search === price.value ? 'selected' : ''}>{price.label}</span>
                                        </div>
                                    </li>
                                );
                            })}
                        <li className="category-item category-item__filter category-item__filter-form">
                            ...
                            <div className="input-group">
                                <input pattern="[0-9]*" placeholder="Giá từ" value={filterPrice.begin_price}
                                       name="begin_price" onChange={handleChange}/>
                                <span>-</span>
                                <input pattern="[0-9]*" placeholder="Giá đến" value={filterPrice.end_price}
                                       name="end_price" onChange={handleChange}/>
                            </div>
