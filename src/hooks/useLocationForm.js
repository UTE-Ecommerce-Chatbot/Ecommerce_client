...
axios.defaults.headers.common["token"] = "c4376de6-e5f5-11ee-b1d4-92b443b7a897";
...
async function fetchInitialData() {
  const { city_id, district_id, ward_id } = (
...
function useLocationForm() {
  ...
  const [shouldFetchInitialLocation, setShouldFetchInitialLocation] =
    useState(false);
  ...
  useEffect(() => {
    (async function () {
      ...
      const options = await (
        await axios.get(
          `https://dev-online-gateway.ghn.vn/shiip/public-api/master-data/district?province_id=${selectedCity.value}`
        )
      ).data.data;
      setState({
        ...state,
  ...
  useEffect(() => {
    (async function () {
      ...
      const options = await (
        await axios.get(
          `https://dev-online-gateway.ghn.vn/shiip/public-api/master-data/ward?district_id=${selectedDistrict.value}`
        )
      ).data.data;
      setState({
        ...state,
